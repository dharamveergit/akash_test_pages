  ...
  return (
    <div className="flex w-full flex-col overflow-hidden rounded-lg border   bg-background2 p-4">
      <div className="flex gap-x-[10px]">
        <div className="flex h-12 w-12 items-center justify-center rounded border  bg-[#FAF9F8] text-xl  font-extrabold uppercase dark:bg-background2">
          {name?.[0]}
          {name?.[1]}
        </div>
        ...
        <div className="flex w-full items-center   justify-between rounded-sm border  p-2">
          ...
          <div className="flex items-center justify-center gap-x-1">
            {gpuModels.slice(0, 1).map((gpu: any, i: any) => (
              <p
                key={i}
                className="rounded-full  border border-[#D1D5DB] bg-[#F4F4F4] px-2   text-2xs  font-bold text-cardGray"
              >
                {gpu}
              </p>
            ))}
            ...
            {gpuModels.length > 2 && (
              <HoverCard>
                <HoverCardTrigger>
                  <p className="cursor-pointer  rounded-full  border border-[#D1D5DB] bg-[#F4F4F4] px-2   text-2xs  font-bold text-cardGray">
                    {`+${gpuModels.length - 1}`}
                  </p>
                </HoverCardTrigger>
                <HoverCardContent>
                  <div className="flex w-52 flex-wrap gap-x-2 gap-y-2 rounded-lg bg-white p-2">
                    {gpuModels.slice(1).map((gpu: any, i: any) => (
                      <p className="rounded-full  border border-[#D1D5DB] bg-[#F4F4F4] px-2   text-2xs  font-bold text-cardGray">
                        {gpu}
                      </p>
                    ))}
                  </div>
